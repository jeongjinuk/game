class Any{
    ...
    getNext(step, operator, random){
        if (this.routeList.length == 0 && random){ // 무한루프 돌 수도 있어서 갈 수 있는 경로에 대한 값을 미리 만들고 인덱스로 랜덤하게 뽑음
            return null;
        }
        let next = [0,0, random ? this.routeList.splice(Math.floor(Math.random() * this.routeList.length), 1)[0] : operator];
        switch (next[2]){
            case 1:
