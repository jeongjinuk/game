...
const isMazeRouteRange = (x, y) => x >= 0 && y >= 0 && x <= size - 1 && y <= size - 1;
// 미로를 더 복잡하게 만들기 위해서
const spliceWalls = (walls) => {
    const setRoad = (coors) => coors.forEach((coor) => maze[coor[0], coor[1]].type = true);
    walls
        .filter(v => {console.log(v.length); return true; })
        .filter(value => value.length > 10) // 벽길이가 40 넘어갈때만
        .forEach(wall => {
...
// 벽 좌표 탐색
const wallSearch = (i, j, copy, coor) => {
    if (isMazeRouteRange(i, j) && !copy[i][j].type) {
        copy[i][j].type = true;
        coor.push([i, j]);
        for (let k = 0; k < 4; k++) {
            wallSearch(i + x[k], j + y[k], copy, coor);
        }
    }
}
...
const setShortestRoute = () => {
    ...
    while (q.length) {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            // }
            if (isMazeRouteRange(nx, ny) && (copy[nx][ny].type != "T" && copy[nx][ny].type)) {
                q.push([nx, ny, pop[2] + 1]);
            }
        }
    }
...
const createdMaze = () => {
    ...
    while (stack.length) {
        ...
        if (isMazeRouteRange(nextRoute[0], nextRoute[1]) &&
...
const increasedDifficulty = () => {
    ...
    for (let i = 0; i < size; i++) {
        for (let j = 0; j < size; j++) {
            if (!copy[i][j].type) {
                const coordinate = [];
                wallSearch(i, j, copy, coordinate);
